...
async def process_pending_assets(page_id: str, owner_id: str) -> int:
    """
    Process all pending assets for a page concurrently.
    Returns the count of successfully processed assets.
    """
    pending = await get_pending_assets_for_page(page_id)
    if not pending:
        return 0
...
async def _process_one(asset: dict, owner_id: str) -> bool:
    ...
    try:
        await update_asset_processing_started(asset_id)

        file_bytes = await _download_from_storage(storage_path)
        if file_bytes is None:
            await mark_asset_failed(asset_id, "Could not download file from storage")
            return False
        ...
        if asset_type == "image":
            ...
            await mark_asset_failed(asset_id, f"Unknown asset_type: {asset_type}")
            return False

    except Exception as e:
        logger.warning("[ASSET PIPELINE] asset=%s error=%s", asset_id, e)
        await mark_asset_failed(asset_id, str(e))
        return False
...
async def _process_document(asset: dict, file_bytes: bytes, owner_id: str) -> bool:
    ...
    # PDF and DOCX extraction is CPU-bound — run in executor to avoid blocking event loop.
    loop = asyncio.get_event_loop()

    if file_type == "application/pdf":
        extraction = await loop.run_in_executor(None, extract_pdf, file_bytes)
    else:
        extraction = await loop.run_in_executor(None, extract_docx, file_bytes, file_type)

    if extraction.error:
        await update_asset_document_result(
            asset_id=asset_id,
...
async def _process_embedded_image(img, parent_asset: dict, owner_id: str) -> None:
    ...
    child_id = await insert_extracted_image_asset(
        page_id=page_id,
        owner_id=owner_id,
    ...
    try:
        ...
        logger.warning("[ASSET PIPELINE] vision failed for extracted image %s: %s", child_id, e)
...
async def _download_from_storage(storage_path: Optional[str]) -> Optional[bytes]:
    ...
    try:
        ...
        logger.warning("[ASSET PIPELINE] Storage download error for %s: %s", storage_path, e)
        return None
...
async def _upload_to_storage(path: str, data: bytes, content_type: str) -> Optional[str]:
    ...
    try:
        db = await get_db()
        await db.storage.from_(SUPABASE_STORAGE_BUCKET).upload(
            path=path,
            file=data,
